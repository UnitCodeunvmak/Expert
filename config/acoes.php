...
if (isset($_POST['editar_livro'])) {
  ...
  $livro = new Livro();
  $result = $livro->editarBook($_POST['id'],$_POST['titulo'], $_POST['autor'], $_POST['editora'], $_POST['ano'], $_POST['categoria']);
  if ($result == 1) {
    $_SESSION['mensagem'] = 'Livro editado com sucesso';
    unset($_SESSION['editar_livro']);
    header('Location: ../views/lista-livro.php');
  } else {
    $_SESSION['mensagem'] = 'Livro não foi editado';
    unset($_SESSION['editar_livro']);
    header('Location: ../views/lista-livro.php');
  }
}
...
if (isset($_POST['deleta_livro'])) {
  ...
  if ($result == 1) {
    ...
    unset($_SESSION['deleta_livro']);
    header('Location: ../views/lista-livro.php');
  } else {
    $_SESSION['mensagem'] = 'Livro não foi deletado';
    unset($_SESSION['deleta_livro']);
    header('Location: ../views/lista-livro.php');
  }
}